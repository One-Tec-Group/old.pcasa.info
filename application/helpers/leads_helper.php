...
function get_leads_summary() {
    ...
    foreach ($statuses as $status) {
        ...
        if (isset($status['lost'])) {
            ...
            $sql .= ' WHERE category=' . $status['id'];
        }
        if (is_admin() || is_admin_normal() || has_permission("leads", '', "view_all")) {
            
        } else {
            $sql .= ' AND assigned=' . get_staff_user_id();
        }
...
function get_phone_number_info($phonenumber) {
    $CI = &get_instance();
    if (!class_exists('leads_model')) {
        $CI->load->model('leads_model');
    }
    ...
    $phone_information = $CI->leads_model->getPhoneNumbers();
    foreach ($phone_information as $single_phone) {
        if ((strlen($single_phone->calling_code) + 1) == strlen($single_phone->phone_code)) {
            if (substr($phonenumber, 0, strlen($single_phone->calling_code)) === $single_phone->calling_code || substr($phonenumber, 0, strlen($single_phone->phone_code)) === $single_phone->phone_code) {
                ...
                echo '<div style="white-space:nowrap;"><img width="35px" class="img-rounded" src="' . base_url("assets/flags/" . $single_phone->flag) . '">&nbsp;&nbsp;<i style="margin-left: 10px;">' . $phonenumber . '</i></div><br/>';
...
function get_lead_nationality($nationality) {
    $CI = &get_instance();
    if (!class_exists('leads_model')) {
        $CI->load->model('leads_model');
    }
    $nationality_information = $CI->leads_model->getNationalities_with_id($nationality);
    if ($nationality_information != null) {
        echo '<div align="center"><br/>'
        . '<img width="50%" class="img-rounded" src="' . base_url("assets/flags/" . $nationality_information->flag) . '"><br/><br/>'
        . '<i>' . $nationality_information->nationality . '</i>'
        . '</div>';
    } else {
