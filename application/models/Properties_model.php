...
class Properties_model extends App_Model
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['client_vault_entries_model', 'client_groups_model', 'statement_model','Properties_model']);
    }
...
public function photo($file) {
  ...
  //  global $types;
          $types = array(
                'images' =>array( // images ext
                        'png','PNG','jpg','jpe','jpeg','gif','bmp','BMP','icon','tiff','tiff','svg','JPEG'
                    ),
                'app' =>array( // compressed ext
                        'zip','rar'
                    ),
                'video'=> array( // video ext
                         '3gp','3gpp2','mp3','mpeg','flv','rmvb','mp4','mov','mpg','3g2','wmv','avi'
                    ),
                'sound'=>array( // sound ext
                         'mp3','mp4','rm','ra','rmvb','avi','wav','wma','m4p',
                    ),
                'doc'=>array( // word & files ext
                         'doc','docx','rtf','xls','xlsx','pdf','ppt','pps','obt','zip','rar'
                    ),
          ...
          // max Width for thumb image
          $max_img_up_thumb = 350;
          $max_img_up_thumb2 = 350;

          // max size for image upload
          $max_img_size_up = 10000000;
  		    ...
  		    $file_type = array('.php', '.exe');
  		    $exten= strtolower(pathinfo($_FILES[$file]['name'], PATHINFO_EXTENSION));
  		    $ext = stristr($file_name, '.');
  		    $random = rand(1, 1000);
  		    $file_name = $file_name;
  	    if ($_FILES[$file]['name'] != "") {
  	        if (!in_array($exten, $types['images'])) {
  		              print '<div class="massage"> امتداد غير مسموح بة   </div>';
  	        } elseif (in_array($ext, $file_type)) {
  	            print '<div class="massage">الملف غير مسموح به</div>';
  	        } elseif ($_FILES[$file]['size'] > $max_img_size_up) {
  		            print '<div class="massage">الملف أكبر من الحجم المطلوب</div>';
  	        } elseif (move_uploaded_file($file_temp, $path . $file_name)) {


  	      $details = getimagesize("$path/$file_name");
  		    $type = preg_replace('@^.+(?<=/)(.+)$@', '$1', $details['mime']);
  		    //$srcImg = imagecreatefrom.$type.("$path/$file_name");
  	    if($type=='jpeg' || $type=='jpg' || $type=='gif' || $type=='png' ){
  		    eval('$srcImg = imagecreatefrom'.$type.'("$path/$file_name");');
  	    }else{
  		    $srcImg = imagecreatefrombmp($path.$file_name);
  		    //eval('$srcImg = imagecreatefrom'.$type.'("$path/$file_name");');
  		    }
  		    // start thum1
  		    $thumbHeight = $details[1] * ($max_img_up_thumb / $details[0]);
  		    $thumbImg = imagecreatetruecolor($max_img_up_thumb, $thumbHeight);
  		    imagecopyresampled($thumbImg, $srcImg, 0, 0, 0, 0, $max_img_up_thumb, $thumbHeight,
  		    $details[0], $details[1]);
  		    $pat_th = __Dir__."/../../upload/content_thumbs/";
  	    if($type=='jpeg'){
  		    //image.$type($thumbImg,$pat_th.$file_name);
  		    eval('image'.$type.'($thumbImg, "$pat_th/$file_name"'.
  		    (($type=='jpeg')?', 90':'').');');}
  		    elseif($type=='png'){
  		    imagePNG($thumbImg,$pat_th.$file_name);
  		    //(($type=='png')?', 80':''));
  		    //eval('imagePNG($thumbImg, "$pat_th/$file_name"'.
  		    //(($type=='png')?', 90':'').');');
  		     }elseif($type=='gif'){
  		    eval('image'.$type.'($thumbImg, "$pat_th/$file_name"'.
  		    (($type=='gif')?', 90':'').');');
  		    //image2wbmp($thumbImg,$pat_th.$file_name);
  		    }else{
  		    //imagePNG($thumbImg,$pat_th.$file_name);
  		    image2wbmp($thumbImg,$pat_th.$file_name);
  		    //eval('image'.$type.'($thumbImg, "$pat_th/$file_name"'.
  		...
  		// end thum 1
  		// start thum 2
  		$thumbHeight2 = 110; //$details[1] * ($max_img_up_thumb2 / $details[0]);
  		$thumbImg2 = imagecreatetruecolor($max_img_up_thumb2, $thumbHeight2);
  		imagecopyresampled($thumbImg2, $srcImg, 0, 0, 0, 0, $max_img_up_thumb2, $thumbHeight2,
  		$details[0], $details[1]);
  		$pat_th2 = __Dir__."/../../upload/content_thumbs/";
  		if($type=='jpeg'){
  		//image.$type($thumbImg,$pat_th.$file_name);
  		eval('image'.$type.'($thumbImg2, "$pat_th2/$file_name"'.
  		(($type=='jpeg')?', 90':'').');');
      }elseif($type=='png'){
  		imagePNG($thumbImg2,$pat_th2.$file_name);
  		//(($type=='png')?', 90':''));
  		//eval('imagePNG($thumbImg, "$pat_th/$file_name"'.
  		//(($type=='png')?', 90':'').');');
  		 }elseif($type=='gif'){
  		...
  		//imagePNG($thumbImg,$pat_th.$file_name);
  		image2wbmp($thumbImg2,$pat_th2.$file_name);
  		//eval('image'.$type.'($thumbImg, "$pat_th/$file_name"'.
  		//(($type==$type)?', 90':'').');');
  		}
  		imagedestroy($srcImg);
  		...
  		// end thumb 2
  		            ...
  		            print '<div class="massage">هناك مشكلة فى رفع الملف حاول مرة أخرى</div>';
  		        }
  		    }
  		}
*/
...
/*
 ...
 /*public function update($table,$updatedArr,$where) {
  $this->db->where($where);
  return $this->db->update($table,$updatedArr,$where);
}
...
/*****************************************************/
    /**
     * Get client object based on passed clientid if not passed clientid return array of all properties
    ...
    {
        $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'properties')) . ',' . get_sql_select_client_company());

        $this->db->join(db_prefix() . 'countries', '' . db_prefix() . 'countries.country_id = ' . db_prefix() . 'properties.country', 'left');
        $this->db->join(db_prefix() . 'contacts', '' . db_prefix() . 'contacts.userid = ' . db_prefix() . 'properties.userid AND is_primary = 1', 'left');
        ...
        if (is_numeric($id)) {
            ...
            $this->db->where(db_prefix() . 'properties.userid', $id);
            $client = $this->db->get(db_prefix() . 'properties')->row();
    ...
    public function update_company_details($data, $id)
    {
        ...
        if ($affectedRows > 0) {
            ...
            log_activity('Customer Info Updated From properties Area [ID: ' . $id . ']');
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('client_id', $id);
            $this->db->update(db_prefix() . 'properties', ['client_id' => 0]);
    ...
    public function get_customer_default_currency($id)
    {
        ...
        $this->db->where('userid', $id);
        $result = $this->db->get(db_prefix() . 'properties')->row();
        if ($result) {
    ...
    public function get_customer_billing_and_shipping_details($id)
    {
        ...
        $this->db->from(db_prefix() . 'properties');
        $this->db->where('userid', $id);
    ...
    {
        $this->db->where('userid', $id);
        $this->db->update(db_prefix() . 'properties', [
            'active' => $status,
        ]);
    ...
    */
    ...
    public function require_confirmation($client_id)
    {
        ...
        $this->db->update(db_prefix() . 'properties', ['active' => 0, 'registration_confirmed' => 0]);
    ...
    public function confirm_registration($client_id)
    {
        $contact_id = get_primary_contact_user_id($client_id);
        $this->db->where('userid', $client_id);
        $this->db->update(db_prefix() . 'properties', ['active' => 1, 'registration_confirmed' => 1]);
    ...
    public function get_properties_distinct_countries()
    {
        return $this->db->query('SELECT DISTINCT(country_id), short_name FROM ' . db_prefix() . 'properties JOIN ' . db_prefix() . 'countries ON ' . db_prefix() . 'countries.country_id=' . db_prefix() . 'properties.country')->result_array();
    }
    ...
    public function send_notification_customer_profile_file_uploaded_to_responsible_staff($contact_id, $customer_id)
    {
        ...
        foreach ($staff as $member) {
            ...
            if (add_notification([
                    ...
                    'link' => 'properties/client/' . $customer_id . '?group=attachments',
                ])) {
    ...
    public function get_status($id = '', $where = []) {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix() . 'properties_status')->row();
        }

        $statuses = $this->app_object_cache->get('properties-all-statuses');

        if (!$statuses) {
            $this->db->order_by('statusorder', 'asc');

            $statuses = $this->db->get(db_prefix() . 'properties_status')->result_array();
            $this->app_object_cache->add('properties-all-statuses', $statuses);
        }
