...
class AccountController extends Controller
{
    ...
    public function actionUsage()
    {
        ...
        if ((int)$customer->getGroupOption('sending.quota_time_value', -1) > -1) {
            ...
            $tsAllowed    = (strtotime(sprintf('+ %d %s', $timeValue, $timeUnit)) - $tsDateAdded);
    ...
    {
        if(isset(Yii::app()->session['backURLFromMarketting']) && isset(Yii::app()->session['FromName'])){
            $logoutUrl = Yii::app()->session['backURLFromMarketting'];
            unset(Yii::app()->session['normalUser']);
            unset(Yii::app()->session['FromName']);
            unset(Yii::app()->session['backURLFromMarketting']);
        }else{
